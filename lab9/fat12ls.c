...
int validfile(unsigned char buffer[], int offset)
{
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + entrywidth)   {
    	if (validfile(buffer, i)) {
    		...
    		// Display Attributes
    		printf("%s\t", parseAttributes(string, (unsigned char)i + attroff)  );
    		// Display Time
            
    		printf("%s\t", parseTime(string,endianSwap(buffer[i+timeoff], buffer[i+timeoff+1]))  );
    		// Display Date
    		printf("%s\t", parseDate(string,endianSwap(buffer[i+dateoff], buffer[i+dateoff+1]))  );
    		// Display Size
...
char * parseAttributes(char string[], unsigned char key)
{
	...
	strcpy(attr, char2bin(string[key]));
	reverse(attr);
